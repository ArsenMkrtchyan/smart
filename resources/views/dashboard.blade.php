...
@section('content')
    <div class="container-fluid py-4">
        ...
        <!-- Վերևի վիճակագրական քարտեր -->
        <div class="row">
            <!-- Քարտ: Ընդհանուր նախագծեր -->
            <div class="col-sm-6 col-md-3 mb-4">
                <div class="card h-100">
                    <div class="card-body">
                        <h5 class="card-title">Ընդհանուր նախագծեր</h5>
                        <h3 class="card-text mb-0">
                            {{ $totalProjects }}
                            <!-- Стрелка и разница vs прошлый месяц -->
                            @php
                        ...
                        <small class="text-muted">համեմատած նախորդ ամսվա հետ</small>
                    </div>
                </div>
            </div>
            <!-- Քարտ: Ընդհանուր եկամուտ -->
            <div class="col-sm-6 col-md-3 mb-4">
                <div class="card h-100">
                    <div class="card-body">
                        <h5 class="card-title">Ընդհանուր եկամուտ</h5>
                        <h3 class="card-text mb-0">{{ number_format($totalIncome, 2, '.', ' ') }}</h3>
                        <small class="text-muted">բոլոր վճարումների գումարը</small>
                    </div>
                </div>
            </div>
            <!-- Քարտ: Ընթացիկ ամսվա եկամուտ -->
            <div class="col-sm-6 col-md-3 mb-4">
                <div class="card h-100">
                    <div class="card-body">
                        <h5 class="card-title">Ընթացիկ ամսվա եկամուտ</h5>
                        <h3 class="card-text mb-0">
                            {{ number_format($currentMonthIncome, 2, '.', ' ') }}
                            @if(isset($incomeGrowthPercent))
                                @if($incomeGrowthPercent !== 0)
                                ...
                                @endif
                            @else
                                <span class="text-muted" style="font-size: 0.9em;"> տվյալներ չկան</span>
                            @endif
                        </h3>
                        <small class="text-muted">նախորդ ամսվա նկատմամբ</small>
                    </div>
                </div>
            </div>
            <!-- Քարտ: Նախորդ ամսվա սպասվող եկամուտ -->
            <div class="col-sm-6 col-md-3 mb-4">
                <div class="card h-100">
                    <div class="card-body">
                        <h5 class="card-title">սպասվող եկամուտ</h5>
                        <h3 class="card-text mb-0">{{ number_format($expectedLastMonth, 2, '.', ' ') }}</h3>
                        <small class="text-muted">{{-- նախորդ ամիս անունում --}}պլանով {{ date('F Y', strtotime('-1 month')) }}</small>
                    </div>
                </div>
            </div>
        </div>

        <!-- Գրաֆիկ 1: Սյունակային գրաֆիկ (Սպասված / Վճարված / Պարտք ըստ ամիսների) -->
        <div class="row mb-5">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Սպասումների և վճարումների համեմատություն ըստ ամիսների {{ $currentYear }}</h5>
                        <div id="chart1"></div>  <!-- տարայք սյունակային գրաֆիկի համար -->
                    </div>
                </div>
            </div>
        </div>

        <!-- Գրաֆիկ 2: Լրացված գրաֆիկ (Գումարներ և գործարքներ) + տարվա և ամսվա ընտրիչներ -->
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex flex-wrap align-items-center mb-3">
                            <h5 class="card-title mb-0 me-3">Վճարումների դինամիկա</h5>
                            <!-- Տարվա ընտրություն -->
                            <label for="yearSelect" class="me-2 mb-0">Տարի:</label>
                            <select id="yearSelect" class="form-select form-select-sm me-3" style="width:auto; display:inline-block;">
                                @foreach($years as $year)
                                    <option value="{{ $year }}" @if($year == $currentYear) selected @endif>
                                        {{ $year }}
                            ...
                            <!-- Ամսվա ընտրություն (ցույց է տալիս տարով ընտրելուց հետո) -->
                            <label for="monthSelect" class="me-2 mb-0">Ամիս:</label>
                            <select id="monthSelect" class="form-select form-select-sm" style="width:auto; display:none;">
                        ...
                        <div id="chart2"></div>  <!-- տարայք լրացված գրաֆիկի համար -->
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Պահանջվում է ApexCharts (CDN) -->
    <script src="https://cdn.jsdelivr.net/npm/apexcharts"></script>

    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // 1. Սյունակային գրաֆիկի կարգավորումներ և գրառում (Գրաֆիկ 1)
            var options1 = {
                chart: {
                    type: 'bar',
                    height: 350,
                ...
                colors: ['#007bff', '#28a745', '#dc3545'],  // կապույտ՝ սպասվածը, կանաչ՝ վճարվածը, կարմիր՝ պարտքը (օր. Bootstrap գույներ)
                dataLabels: {
                    enabled: false
                },
                stroke: {
                ...
                yaxis: {
                    title: { text: 'Գումար, ₽' }  // կարգավորեք անհրաժեշտ արժույթը
                },
                fill: {
                    opacity: 1
                },
                tooltip: {
                    y: {
                        formatter: function(val) {
                            return val.toFixed(2);  // թռուցիկի ձևաչափավորում (2 տասնորդական)
                        }
                    }
                },
                legend: {
            ...
            // 2. Լրացված գրաֆիկի սկզբնական կարգավորումներ և գրառում (Գրաֆիկ 2)
            var options2 = {
                chart: {
                    type: 'area',
                    height: 350,
                ...
                yaxis: [
                    {
                        title: { text: 'Վճարումների գումար, ₽' }
                    },
                    {
                        opposite: true,
                        title: { text: 'Գործարքների քանակը' }
                    }
                ],
                tooltip: {
                    shared: true,
            ...
            // 3. Գրաֆիկ 2-ի դինամիկ թարմացման լոգիկա կարգավորիչների փոփոխության դեպքում
            var yearSelect = document.getElementById('yearSelect');
            var monthSelect = document.getElementById('monthSelect');

            // URL կազմավորում chartData երթուղու (Laravel route) համար և տվյալների բեռնում
            function loadChartData(selectedYear, selectedMonth = '') {
                var url = "{{ route('dashboard.chartData') }}" + "?year=" + selectedYear;
                if (selectedMonth) {
                    url += "&month=" + selectedMonth;
                ...
                fetch(url)
                    ...
                    .then(data => {
                        if (data.error) {
                            console.error("Գրաֆիկի տվյալների բեռնումի սխալ:", data.error);
                            return;
                        }
                        chart2.updateOptions({
                            xaxis: {
                                categories: data.categories,
                                title: { text: selectedMonth ? 'Օրեր' : 'Ամիսներ' }
                            },
                            series: [
                                { name: 'Amount Paid', data: data.sums },
                                { name: 'Transactions', data: data.counts }
                    ...
                    .catch(err => console.error("AJAX սխալ:", err));
            }
