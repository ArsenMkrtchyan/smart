...
@section('content')
    <div class="container">
        ...
        <table class="table table-bordered">
            ...
            @foreach($groupedPayments as $date => $paymentsGroup)
                @php
                    // Հաշվում ենք այդ օրվա վճարումների ընդհանուր գումարը
                    $totalAmount = $paymentsGroup->sum('amount');
                    // collapse-ի ճիշտ id ձևավորման համար օգտագործում ենք даты-ից slug
                    $collapseId = \Illuminate\Support\Str::slug($date);
                @endphp
                <tr>
                    <td>{{ $date }}</td>
...
<!-- Կիցում ենք Bootstrap CSS (եթե դեռ չկիցված) -->
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">

<!-- Կիցում ենք jQuery (եթե դեռ չկիցված) -->
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<!-- Կիցում ենք Bootstrap JS -->
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.bundle.min.js"></script>
